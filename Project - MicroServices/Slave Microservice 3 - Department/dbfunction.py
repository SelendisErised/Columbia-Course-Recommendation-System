...
class ExtraInfo(Tools):
    def __init__(self, default_scheme, cur, conn):
        self.database_name = default_scheme
        self.db_cursor = cur
        self.conn = conn

    def prefix_to_department(self, course_prefix):
        mysql = "select department from {0}.{1} where Course_Prefix = '{2}'".format(
            self.database_name, 'course_to_department', course_prefix)
        self.db_cursor.execute(mysql)
        query_output = self.db_cursor.fetchall()

        json_out = json.dumps(query_output)
